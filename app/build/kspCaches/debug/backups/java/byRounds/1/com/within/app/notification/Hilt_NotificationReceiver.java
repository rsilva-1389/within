  ...
  protected void inject(Context context) {
    if (!injected) {
      synchronized (injectedLock) {
        if (!injected) {
          ((NotificationReceiver_GeneratedInjector) BroadcastReceiverComponentManager.generatedComponent(context)).injectNotificationReceiver(UnsafeCasts.<NotificationReceiver>unsafeCast(this));
          injected = true;
        }
      }
    }
